...
open class Terminal {
    ...
    // Safety net: if a program opens a synchronized-output batch (DECSET 2026)
    // but never closes it (lost `2026l`, crash mid-frame), force the batch to
    // end after this long so the display can never stay frozen. Kept short so a
    // dropped close is barely perceptible instead of a multi-second blank.
    private let synchronizedOutputTimeoutSeconds: TimeInterval = 0.25
    ...
    /// True while a synchronized-output batch (DECSET 2026) is open. The view
    /// suppresses repaints during a batch and flushes once it closes, so a
    /// partially-drawn frame is never shown.
    var inSynchronizedOutput: Bool { synchronizedOutputActive }
...
//                break
    ...
    {
        // Nested/repeated opens are ignored: we keep painting the live buffer
        // (just deferred) rather than freezing a snapshot, and we must NOT
        // reschedule the timeout on every open — a program that streams `2026h`
        // faster than the timeout would otherwise push the deadline forever and
        // keep the screen frozen. The single timeout from the initial open
        // guarantees the batch always ends.
        guard !synchronizedOutputActive else { return }
