...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// from the view's frame — used to mirror a remote companion's dimensions.
    /// Set to nil to resume frame-based sizing.
    ...
    /// Honor OSC 52 clipboard writes from the running program. Mouse-reporting
    /// TUIs (opencode, grok, anything on Bubble Tea) capture the drag and copy
    /// the selection via OSC 52 instead of letting the terminal select — without
    /// this the "copied N chars" never actually reaches the macOS clipboard.
    open func clipboardCopy(source: Terminal, content: Data) {
        guard let str = String(data: content, encoding: .utf8), !str.isEmpty else { return }
    ...
    open func linefeed(source: Terminal) {
        // Preserve the manual selection while output streams in (Warp-style), even for
        // mouse-reporting TUIs like Claude Code. Selection positions are stored in absolute
        // buffer coordinates, so they stay anchored to the same text as the buffer scrolls.
    }
    ...
    /// Holding Shift forces a local text selection even when the running program
    /// has mouse reporting enabled (xterm/iTerm2/Terminal.app convention). Without
    /// this, drag events in TUIs like opencode/grok are consumed by the app, so
    /// SwiftTerm never builds a selection (and Copy / Summarize have nothing).
    private func forceLocalSelection(_ event: NSEvent) -> Bool {
        event.modifierFlags.contains(.shift)
    }

    public override func mouseDown(with event: NSEvent) {
        if allowMouseReporting && terminal.mouseMode.sendButtonPress() && !forceLocalSelection(event) {
    ...
    public override func mouseUp(with event: NSEvent) {
        ...
        if allowMouseReporting && terminal.mouseMode.sendButtonRelease() && !forceLocalSelection(event) {
            sharedMouseEvent(with: event)
            return
        }
    ...
    public override func mouseDragged(with event: NSEvent) {
        ...
        if allowMouseReporting && !forceLocalSelection(event) {
    ...
    /// Accumulates sub-line pixel deltas from precise (trackpad / Magic Mouse)
    ...
    public override func scrollWheel(with event: NSEvent) {
        ...
        // presses (buttons 4/5) instead of scrolling our local scrollback —
        // otherwise the wheel does nothing because there is no scrollback to move.
        if allowMouseReporting && terminal.mouseMode.sendButtonPress() {
            let lines: Int
            if event.hasPreciseScrollingDeltas {
                preciseScrollAccumulator += event.scrollingDeltaY
                let h = max(cellDimension.height, 1)
                lines = Int((preciseScrollAccumulator / h).rounded(.towardZero))
                if lines == 0 { return }
                preciseScrollAccumulator -= CGFloat(lines) * h
            } else {
                if event.deltaY == 0 { return }
                lines = Int(event.deltaY.rounded(event.deltaY > 0 ? .up : .down))
            }
            let flags = event.modifierFlags
            ...
            let hit = calculateMouseHit(with: event)
            let screenRow = max(0, min(terminal.displayBuffer.rows - 1, hit.grid.row - terminal.displayBuffer.yDisp))
            for _ in 0 ..< min(abs(lines), terminal.rows) {
                terminal.sendEvent(buttonFlags: buttonFlags, x: hit.grid.col, y: screenRow, pixelX: hit.pixels.col, pixelY: hit.pixels.row)
            }
            return
        }
        // Precise scrolling (trackpad) reports its movement in `scrollingDeltaY`
        // with `deltaY == 0`, so the legacy `deltaY` path below ignores it.
        // Convert the pixel delta into whole lines using the cell height.
        if event.hasPreciseScrollingDeltas {
            preciseScrollAccumulator += event.scrollingDeltaY
            let h = max(cellDimension.height, 1)
            let lines = Int((preciseScrollAccumulator / h).rounded(.towardZero))
            if lines == 0 { return }
            preciseScrollAccumulator -= CGFloat(lines) * h
            if lines > 0 {
