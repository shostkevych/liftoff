...
extension TerminalView {
    ...
    func processSizeChange (newSize: CGSize) -> Bool {
        let newRows: Int
        let newCols: Int
        if let forced = forcedGridSize {
            newCols = forced.cols
            newRows = forced.rows
        } else {
            newRows = Int (newSize.height / cellDimension.height)
            newCols = Int (getEffectiveWidth (size: newSize) / cellDimension.width)
        }
    ...
    func buildAttributedString (row: Int, line: BufferLine, cols: Int) -> ViewLineInfo
    {
        ...
        while col < cols {
            ...
            if isSelected {
                ...
                mutable[.foregroundColor] = TTColor.black
    ...
    {
        search.invalidate()
        // Always preserve the manual selection while output streams in (Warp-style),
        // even for mouse-reporting TUIs like Claude Code.
        startDisplayUpdates()
    }
