...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    public var forcedGridSize: (cols: Int, rows: Int)? {
        didSet { _ = processSizeChange(newSize: bounds.size) }
    }
    ...
    // iOS 17+ inline predictions are ON by default and are NOT covered by
    // autocorrectionType. They feed erratic insertText calls into our custom
    // text storage and get accepted on space/return — in a terminal that means
    // a stray newline ("send") mid-typing. Force them off. SwiftTerm targets
    // pre-iOS-17, so the protocol requirement is satisfied via an availability
    // guarded computed property backed by untyped storage.
    private var _inlinePredictionType: Any?
    @available(iOS 17.0, *)
    public var inlinePredictionType: UITextInlinePredictionType {
        get { (_inlinePredictionType as? UITextInlinePredictionType) ?? .no }
        set { _inlinePredictionType = newValue }
    }
